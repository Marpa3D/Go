...
func main() {
	...
	// nil - это УКАЗАТЕЛЬ НА НЕСУЩЕСТВУЮЩИЙ ТИП ДАННЫХ, В НИКУДА!!!
	...
	// 8. Удаление пары из мапы
	// 8.1 Удаление существующей пары
	fmt.Println("Карта до удаления: ", employee)
	delete(employee, "Dan")
	fmt.Println("Карта (мапа) после удаления пары: ", employee)

	// 8.2 Удаление несуществующей пары
	if _, ok := employee["Alice"]; ok {
		delete(employee, "Alice") // Это ОЧЕНЬ ДОРОГАЯ ОПЕРАЦИЯ!!! Лучше так не делать.
	}

	fmt.Println("Карта (мапа) после удаления пары: ", employee)

	// 9 map - это ссылочный тип. Все новые значения у других мап, связанных с родительской, тоже изменяются.
	// и наоборот - если меняется потомок, то и родитель.
	w := map[string]string{
		"One": "Один",
		"Two": "Два",
	}
	...
	// 10. Сравнивать мапу можно только с nil, т.к. это ссылочный тип!
}
